...
void loop() {
 ...
 //Serial.println();
    ...
    if ((ID & 0x00FF0000) == 0x00EA0000){
        ...
        if (rxmsg.buf[0] == 0xEC && rxmsg.buf[1] == 0xFE){
          requestSendTimer = 0;
          sendVIN(VIN);
        }
//        else if (rxmsg.buf[0] == 0xEB && rxmsg.buf[1] == 0xFE){
//          requestSendTimer = 0;
...
void sendVIN(char id[100])
{
       txmsg.ext = 1;
       txmsg.len = 8;
  
       Serial.print("Received Request for VIN. Sending  ");
       for (int i = 0; i < 35;i++) Serial.print(id[i]);
       Serial.println();
       byte transport0[8] = {32,17,0,3,0xFF,0xEC,0xFE,0};
       byte transport1[8] = {1,id[0],id[1],id[2],id[3],id[4],id[5],id[6]};
       byte transport2[8] = {2,id[7],id[8],id[9],id[10],id[11],id[12],id[13]};
       byte transport3[8] = {3,id[14],id[15],id[16],0xFF,0xFF,0xFF,255};
       txmsg.id = 0x18ECFF18; 
       //Priority 0x18 -> 6, 
       //PGN 0x00EC00 Transport Protocol Connection Management (TP.CM), 
       //Destination Address -> 0xFF (Global), 
       //Source address -> 0x18 = 24 (Trip recorder)
       for (int i = 0;i<8;i++) txmsg.buf[i]=transport0[i];
       CANbus.write(txmsg);
       delay(3);
       txmsg.id = 0x18EBFF18;
       //Priority 0x18 -> 6, 
       //PGN 0x00EB00 Transport Protocol Data Transfer (TP.DT), 
       //Destination Address -> 0xFF (Global), 
       //Source address -> 0x18 = 24 (Trip recorder)
       for (int i = 0;i<8;i++) txmsg.buf[i]=transport1[i];
       CANbus.write(txmsg);
       ...
       delay(3);
       for (int i = 0;i<8;i++) txmsg.buf[i]=transport3[i];
       if (CANbus.write(txmsg)) Serial.print("Sent Last Data Message for VIN");
